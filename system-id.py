...
class SystemId:
    def __init__(self):
        ...
        self.home_pos_np = np.array([-350, -25, 82, 180, 0, 0])
    ...
    def execute_trajectory(self, trajectory_ix):
        for i in range(self.trajectories_np[trajectory_ix].shape[0]):
            self.run_point(
                self.d.MovL(*self.trajectories_np[trajectory_ix][i].tolist(), coordinateMode=0, speed=3)
            )
    ...
    def run_home_pos(self):
        self.run_point(self.d.MovJ(*self.home_pos_np.tolist(), coordinateMode=0, v=10))
